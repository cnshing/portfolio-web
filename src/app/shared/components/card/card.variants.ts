...
export const cardVariants = cva('rounded-md border border-color-default bg-color-surface1 text-color-primary shadow-sm min-w-fit w-full p-lg flex flex-col gap-y-xs', {
  variants: {},
});
export type ZardCardVariants = VariantProps<typeof cardVariants>;

export const cardHeaderVariants = cva('flex flex-wrap gap-x-sm gap-y-xs items-center', {
  variants: {},
});
